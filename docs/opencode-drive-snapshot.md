# OpenCode migration snapshot via gd-sync-harness

This workflow creates a stable Google Drive snapshot of an existing OpenCode config tree for migration analysis.

The `gd-sync-harness` repository builds the `knowledge-sync` CLI. Its profile source root is treated as the logical Gitignore root even when the source is not a Git repository. Root and nested `.gitignore` files are captured into a committed synchronization-policy snapshot. Global Git excludes and `.git/info/exclude` are not used.

Important behavior:

- `knowledge-sync profile add` captures the current `.gitignore` policy before the first synchronization is queued.
- Changing `.gitignore` later does not immediately change the active policy; use `knowledge-sync profile ignore update <profile>`.
...
- If `summary.externalSymlinks` is greater than zero, stop and decide how those targets should be captured. `knowledge-sync` will not upload the symlink targets through the source link.
...
```bash
knowledge-sync profile ignore status opencode-migration-snapshot
knowledge-sync profile status opencode-migration-snapshot
```
...
```bash
knowledge-sync profile disable opencode-migration-snapshot
```
...
```bash
knowledge-sync profile ignore update opencode-migration-snapshot
knowledge-sync profile wait opencode-migration-snapshot
```
