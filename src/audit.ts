...
const HIGH_CONFIDENCE_SECRETS: Array<{ code: string; label: string; pattern: RegExp }> = [
  ...
  {
    code: 'github-token-content',
    label: 'GitHub token-shaped literal',
    pattern: /\b(?:gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,})\b/,
  },
  {
    code: 'aws-access-key-content',
    label: 'AWS access-key-shaped literal',
    pattern: /\b(?:AKIA|ASIA)[A-Z0-9]{16}\b/,
  },
  {
    code: 'slack-token-content',
    label: 'Slack token-shaped literal',
    pattern: /\bxox[baprs]-[A-Za-z0-9-]{10,}\b/,
  },
  {
    code: 'openai-key-content',
    label: 'API key-shaped sk- literal',
    pattern: /\bsk-[A-Za-z0-9_-]{20,}\b/,
  },
  {
    code: 'google-api-key-content',
    label: 'Google API key-shaped literal',
    pattern: /\bAIza[0-9A-Za-z_-]{35}\b/,
  },
  {
    code: 'credential-url-content',
    label: 'URL containing inline credentials',
    pattern: /https?:\/\/[^/\s:@]+:[^/\s@]+@/,
  },
  {
    code: 'bearer-token-content',
    label: 'Bearer token-shaped literal',
...
function repoRelative(repoRoot: string, path: string): string {
  const value = toPosix(relative(repoRoot, path));
  return value || '.';
}
...
function addFinding(audit: MutableRepoAudit, finding: AuditFinding): void {
  if (audit.findings.some(item => item.code === finding.code && item.path === finding.path)) return;
...
function sensitivePathFinding(relPath: string): AuditFinding | null {
  ...
  if ((base === '.env' || base.startsWith('.env.')) && !envSafe.has(base)) {
    return {
      severity: 'blocker',
      code: 'sensitive-env-path',
      path: relPath,
      detail: 'environment file may contain credentials or private configuration',
      suggestedIgnore: base === '.env' ? '.env' : '.env.*',
    };
  }

  if (base === '.netrc' || ['id_rsa', 'id_ed25519', 'id_ecdsa', 'id_dsa'].includes(base)) {
    return {
      severity: 'blocker',
      code: 'sensitive-credential-path',
      path: relPath,
      detail: 'credential/private-key filename must be reviewed before Git initialization',
      suggestedIgnore: `/${relPath}`,
    };
  }

  if (
    /^(?:credentials?|service-account|service_account|tokens?|cookies?|storage-state|storage_state|auth-state|auth_state)(?:[._-].*)?\.json$/i.test(base)
    || /^secrets?(?:[._-].*)?$/i.test(base)
  ) {
    return {
      severity: 'blocker',
      code: 'sensitive-state-path',
      path: relPath,
      detail: 'credential/session/state filename may contain private material',
      suggestedIgnore: `/${relPath}`,
    };
  }

  if (ext === '.key' || ext === '.p12' || ext === '.pfx') {
    return {
      severity: 'blocker',
      code: 'sensitive-key-path',
      path: relPath,
      detail: `${ext} files commonly contain private key material`,
      suggestedIgnore: `/${relPath}`,
    };
  }

  if (ext === '.pem' || base === '.npmrc' || base === '.pypirc') {
    return {
      severity: 'review',
      code: 'credential-config-path',
      path: relPath,
      detail: 'file type may be safe or may contain credentials; inspect before first commit',
    };
  }
...
function shouldScanContent(path: string, size: number): boolean {
  if (size > MAX_CONTENT_SCAN_BYTES) return false;
  const base = basename(path);
  if (['Dockerfile', 'Makefile', 'Justfile'].includes(base)) return true;
...
function scanTextContent(audit: MutableRepoAudit, relPath: string, text: string): void {
  ...
  for (const match of text.matchAll(literal)) {
    ...
    addFinding(audit, {
      severity: 'review',
      code: 'credential-like-literal',
      path: relPath,
      detail: 'credential-like assignment contains a non-placeholder literal; value is intentionally not shown',
    });
    break;
  }
  ...
  if (/\/Users\/[^/\s'"`]+\/|\/home\/[^/\s'"`]+\/|[A-Za-z]:\\Users\\[^\\\s'"`]+\\/.test(text)) {
    addFinding(audit, {
      ...
      path: relPath,
      detail: 'contains an absolute user-home path; review for privacy and portability',
    });
  }
}
...
async function scanEntry(audit: MutableRepoAudit, repoRoot: string, path: string): Promise<void> {
  ...
  if (entryStat.isSymbolicLink()) {
    ...
    try {
      ...
      addFinding(audit, {
        ...
        code: 'broken-symlink',
        path: relPath,
        detail: 'symlink target does not currently resolve',
      });
      return;
    }
    ...
    if (isAbsolute(rawTarget)) {
      addFinding(audit, {
        ...
        path: relPath,
        detail: 'absolute symlink is machine-specific and may expose a local path',
      });
    } else if (!within(repoRoot, resolvedTarget)) {
      addFinding(audit, {
        severity: 'review',
        code: 'external-symlink',
        path: relPath,
        detail: 'relative symlink resolves outside the repository',
      });
    }
    return;
  }
  ...
  if (entryStat.isDirectory()) {
    ...
    if (name === '.git') {
      addFinding(audit, {
        severity: relPath === '.git' ? 'review' : 'blocker',
        code: relPath === '.git' ? 'git-already-initialized' : 'embedded-git',
        path: relPath,
        detail: relPath === '.git'
          ? 'repository already contains Git metadata'
          : 'nested Git metadata would create ambiguous provenance or an embedded-repository gitlink',
      });
      return;
    }
    ...
    if (ignorePattern) {
      ...
      if (LOCAL_RUNTIME_ENV_DIRS.has(name)) {
        addFinding(audit, {
          severity: 'review',
          code: 'local-runtime-environment',
          path: relPath,
          detail: 'local virtual environment should not be committed; ensure dependencies are reproducible without it',
        });
      }
      return;
    }
    ...
    for (const entry of await readdir(path, { withFileTypes: true })) {
      await scanEntry(audit, repoRoot, join(path, entry.name));
    }
    return;
  }
  ...
  if (base === '.git' && relPath !== '.git') {
    addFinding(audit, {
      severity: 'blocker',
      code: 'embedded-git',
      path: relPath,
      detail: 'nested .git file indicates embedded Git metadata or worktree linkage',
    });
  }

  if (entryStat.size >= BLOCKER_BYTES) {
    addFinding(audit, {
      severity: 'blocker',
      code: 'oversize-file',
      path: relPath,
      detail: `file is ${(entryStat.size / 1024 / 1024).toFixed(1)} MiB (GitHub hard limit risk)`,
    });
  } else if (entryStat.size >= REVIEW_BYTES) {
    addFinding(audit, {
      severity: 'review',
      code: 'large-file',
      path: relPath,
      detail: `file is ${(entryStat.size / 1024 / 1024).toFixed(1)} MiB; confirm it belongs in source control`,
    });
  }

  if (shouldScanContent(path, entryStat.size)) {
    const text = await readFile(path, 'utf8').catch(() => '');
    if (text) scanTextContent(audit, relPath, text);
  }
}
...
async function readPlan(planPath: string): Promise<MinimalMigrationPlan> {
  ...
  const repositories = value.repositories.map((item, index): MinimalRepoPlan => {
    ...
    if (basename(repo.id) !== repo.id || repo.id === '.' || repo.id === '..' || !/^[A-Za-z0-9._-]+$/.test(repo.id)) {
      throw new Error(`Unsafe repository id in migration plan: ${repo.id}`);
    }
...
async function auditRepo(repoId: string, targetRoot: string): Promise<RepoAudit> {
  ...
  const mutable: MutableRepoAudit = {
    repoId,
    repoPath,
    exists: repoExists,
    gitignorePresent: false,
    stats: { files: 0, directories: 0, symlinks: 0, bytes: 0, prunedNoiseDirectories: 0 },
    findings: [],
    ignoreMap: new Map(),
    existingIgnorePatterns: new Set(),
  };
  ...
  if (!repoExists) {
    addFinding(mutable, {
      ...
      path: '.',
      detail: 'planned migrated repository does not exist at the target root',
    });
  } else {
    ...
    if (!rootStat.isDirectory()) {
      addFinding(mutable, {
        ...
        path: '.',
        detail: 'planned repository path is not a directory',
      });
    } else {
      const gitignore = join(repoPath, '.gitignore');
      if (await exists(gitignore)) {
        mutable.gitignorePresent = true;
        mutable.existingIgnorePatterns = parseExistingIgnore(await readFile(gitignore, 'utf8'));
      }
      for (const entry of await readdir(repoPath, { withFileTypes: true })) {
        await scanEntry(mutable, repoPath, join(repoPath, entry.name));
      }
    }
  }

  for (const finding of mutable.findings) {
    if (finding.suggestedIgnore && !mutable.existingIgnorePatterns.has(finding.suggestedIgnore)) {
      addIgnore(mutable, finding.suggestedIgnore, finding.path);
    }
  }

  const ignoreCandidates = [...mutable.ignoreMap.entries()]
    .map(([pattern, paths]) => ({ pattern, paths: [...paths].sort() }))
    .sort((left, right) => left.pattern.localeCompare(right.pattern));
  const findings = [...mutable.findings].sort((left, right) => {
  ...
  const readyForInitialCommit = findings.length === 0 && ignoreCandidates.length === 0;

  return {
    repoId,
    repoPath,
    exists: mutable.exists,
    gitignorePresent: mutable.gitignorePresent,
    stats: mutable.stats,
    findings,
    ignoreCandidates,
    readyForInitialCommit,
  };
}
...
export function renderMigrationAudit(result: MigrationAuditResult): string {
  ...
  for (const repo of result.repositories) {
    ...
    lines.push(
      `${repo.repoId}: ${status} (${blockers} blocker, ${reviews} review, ${repo.ignoreCandidates.length} ignore suggestion)`,
    );
    for (const finding of repo.findings) {
      lines.push(`  [${finding.severity === 'blocker' ? 'BLOCK' : 'REVIEW'}] ${finding.path} — ${finding.code}: ${finding.detail}`);
    }
    for (const candidate of repo.ignoreCandidates) {
      const example = candidate.paths.slice(0, 3).join(', ');
      const more = candidate.paths.length > 3 ? ` (+${candidate.paths.length - 3} more)` : '';
      lines.push(`  [IGNORE] ${candidate.pattern} — observed: ${example}${more}`);
    }
  }

  lines.push(
    `Summary: ${result.summary.blockers} blocker(s), ${result.summary.reviews} review item(s), ${result.summary.ignorePatterns} ignore suggestion(s)`,
  );
